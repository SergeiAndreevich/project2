...
describe('test posts', ()=>{
    ...
    it('should not create a new post', async () => {
        const res = await request(app).post(PATH.posts).send(invalidPostSet).expect(httpStatus.BadRequest);
        expect(res.body.errorMessages.length).toBe(5); // 2 шт получается в blodId из-за того что пустой и не нумерик стринг
        const post = await request(app).get(PATH.posts).expect(httpStatus.Ok);
        expect(post.body.length).toBe(0)
    });
    it('should not create a new post', async () => {
        const res = await request(app).post(PATH.posts).send({...invalidPostSet, title: '    b   b   ', shortDescription: '    a', content: 'b   ', blogId: 'a'})
            .expect(httpStatus.BadRequest);
        expect(res.body.errorMessages.length).toBe(1)
    });
    it('should not create a new post', async () => {
        const res = await request(app).post(PATH.posts).send({...invalidPostSet,  title:1, shortDescription:2, content:3, blogId:4}).expect(httpStatus.BadRequest);
